...
class FirebaseLogger {
  ...
  void addKeyValue({required String key, required String? value}) {
    _parameter[key] = value != null
        ? value.length > _kEndIndex
            ? value.substring(0, _kEndIndex)
            : value
        : "";
  }
  ...
  void addBoolValue({required String key, required bool? value}) {
    _parameter[key] = value != null ? value.toString() : "false";
  }
  ...
  void addUserId() {
    _parameter["user_id"] = getLoginProvider().userId;
  }
  ...
  void addDoubleWithPercentValue(
      {required String key, required double? value}) {
    _parameter[key] = value != null ? "${value.toStringAsFixed(2)}%" : "0.00%";
  }

  void addKeyWithPercentValue({required String key, required String value}) {
    double result = double.tryParse(value) ?? 0;
    _parameter[key] = result != 0 ? "${result.toStringAsFixed(2)}%" : "0.00%";
  }

  void publishToSuperApp(String eventName) {
    if (AppConfig().configModel.firebaseEnabled.toLowerCase() == "false" ||
