...
class CarGalleryDetailScreen extends StatefulWidget {
  final List<CarGalleryModel>? imageList;
  final int initialPage;

  const CarGalleryDetailScreen({
    Key? key,
    this.imageList,
    this.initialPage = _defaultPage,
  }) : super(key: key);
  @override
  CarGalleryDetailScreenState createState() => CarGalleryDetailScreenState();
}

class CarGalleryDetailScreenState extends State<CarGalleryDetailScreen>
  ...
  Widget _buildArrowIcon(bool isLeftArrow, PageController controller) {
    return IconButton(
      key: isLeftArrow ? const Key(_kLeftIconKey) : const Key(_kRightIconKey),
      icon: Icon(
        isLeftArrow
  ...
  Widget _buildPageViewBuilder(PageController controller) {
    return PageView.builder(
      ...
      itemBuilder: (context, position) {
        return _buildPhotoView(widget.imageList![position].full);
      },
      onPageChanged: (index) {
        setState(() {
          _currentPage = index;
