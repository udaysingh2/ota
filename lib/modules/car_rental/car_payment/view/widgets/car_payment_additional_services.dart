...
class CarPaymentAdditionalServices extends StatelessWidget {
  final bool? showDivider;
  final double? totalAdditionalServicePrice1;
  final int? numberOfServices1;
  ...
  CarPaymentAdditionalServices({
    Key? key,
    this.showDivider = true,
    this.totalAdditionalServicePrice1,
    this.numberOfServices1,
  ...
  Widget build(BuildContext context) {
    List<ExtraChargeData> additionalAddon =
        extraCharge!.where((element) => element.isCompulsory == false).toList();
    ...
    return Column(
      ...
      children: [
        ...
        ListView.builder(
            itemCount: additionalAddon.length,
            physics: const NeverScrollableScrollPhysics(),
            shrinkWrap: true,
            itemBuilder: (_, index) {
              return _getAdditionalData(context, additionalAddon[index], index,
                  additionalAddon.length);
            }),
      ],
    );
  }
  ...
  Widget _getAdditionalServiceHeader(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.only(left: kSize24, bottom: kSize16),
      child: Text(
...
//                 _currencyUtil
//                     .getFormattedPrice(extraCharge.addonPriceToDisplay ?? 0)),
