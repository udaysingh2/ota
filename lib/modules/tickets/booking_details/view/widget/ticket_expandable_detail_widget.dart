...
class TicketExpandableDetailWidget extends StatelessWidget {
  final String? title;
  final List<TicketBookingDetailsHighlight>? facilityMap;
  final List<TicketBookingDetailsTicketTypeInfo> ticketType;
  final TourExpandableController controller;

  const TicketExpandableDetailWidget({
    Key? key,
    this.title,
    this.facilityMap,
  ...
  Widget _getFacilityView(
      BuildContext context, List<TicketBookingDetailsHighlight>? facilityMap) {
    return Column(
  ...
  List<Widget> _getFacilityList(
    ...
    if (facilityMap != null && facilityMap.isNotEmpty) {
      for (TicketBookingDetailsHighlight facility in facilityMap) {
