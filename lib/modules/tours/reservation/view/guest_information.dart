...
import 'package:ota/modules/tours/reservation/bloc/guest_information_bloc.dart';
...
class GuestInformation extends StatefulWidget {
  const GuestInformation({Key? key}) : super(key: key);

  @override
  GuestInformationState createState() => GuestInformationState();
}

class GuestInformationState extends State<GuestInformation> {
  ...
  GuestInformationArgumentModel get _guestInfoArgumentModel =>
      ModalRoute.of(context)?.settings.arguments
          as GuestInformationArgumentModel;
  final GuestInformationBloc _guestInformationBloc = GuestInformationBloc();
    ...
    return Row(
      ...
      children: [
        Text(
          getTranslated(
                context,
                (_guestInfoArgumentModel.isForAdult
                    ? AppLocalizationsStrings.adult
                    : AppLocalizationsStrings.child),
              ).addTrailingSpace() +
  ...
  _buildDatePicker({
    ...
    if (data != null) {
      ...
      if (isForDOB) {
        _dobController.text = Helpers.getddMMyyyy(dateTime);
      } else {
        _passportExpirationDateController.text = Helpers.getddMMyyyy(dateTime);
      }
