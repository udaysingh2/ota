import 'dart:convert';

import 'package:flutter_test/flutter_test.dart';
import 'package:graphql_flutter/graphql_flutter.dart';
import 'package:ota/core_pack/graphql/graphql_client.dart';
import 'package:ota/domain/popup/data_sources/popup_remote_data_source.dart';

import '../../../mocks/fixture_reader.dart';

class MockPopupGraphQl implements GraphQlResponse {
  Map<String, dynamic> map = json.decode(fixture("popup/popup.json"));
  @override
  Future<QueryResult> getGraphQlResponse(
      {String query = '',
      //This is mandatory for crashlytics
      required String queryName,
      FetchPolicy fetchPolicy = FetchPolicy.networkOnly,
      String? acceptLanguage,
      String? bookingUrn,
      bool authRequired = true}) async {
    QueryResult queryResult = QueryResult(
      source: QueryResultSource.optimisticResult,
      data: map,
      options: QueryOptions(
        fetchPolicy: fetchPolicy,
        document: gql(query),
      ),
    );
    return queryResult;
  }
}

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  group(" popUp remote data source group", () {
    test(' popUp test', () async {
      PopupRemoteDataSource popupRemoteDataSource = PopupRemoteDataSourceImpl();
      popupRemoteDataSource =
          PopupRemoteDataSourceImpl(graphQlResponse: MockPopupGraphQl());
      PopupRemoteDataSourceImpl.setMock(MockPopupGraphQl());
      popupRemoteDataSource.getPopup();
    });
  });
}
